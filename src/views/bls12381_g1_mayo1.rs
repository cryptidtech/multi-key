...
//! BLS12-381-G1-MAYO-1 hybrid signing multikey view (Birds-of-Prey-1);
//! combines BLS12-381 G1 with MAYO-1 post-quantum multivariate signatures.
//! Sign: s1 = BLS_G1(m), s2 = Mayo1(m || s1), sig = s1 || s2
//! Verify: verify BLS_G1(m, s1) && verify Mayo1(m || s1, s2)
//! Secret layout: bls_g1_secret (32) || mayo1_seed (24) = 56 bytes.
//! Public encoding is classical-first: bls_g1_pub (96) || mayo1_pub.
...
const MAYO1_SEED_LEN: usize = 24;
const PRIV_SEED_LEN: usize = bls::BLS_G1_SECRET_LEN + MAYO1_SEED_LEN; // 56
const MAYO1_PUB_LEN: usize = 1420;
const PUB_KEY_LEN: usize = bls::BLS_G1_PUB_LEN + MAYO1_PUB_LEN; // 1516
const MAYO1_SIG_LEN: usize = 454;
const HYBRID_SIG_LEN: usize = bls::BLS_G1_SIG_LEN + MAYO1_SIG_LEN; // 502
...
impl<'a> AttrView for View<'a> {
    ...
    fn is_secret_key(&self) -> bool {
        self.mk.codec == Codec::Bls12381G1Mayo1Priv
    }
    fn is_public_key(&self) -> bool {
        self.mk.codec == Codec::Bls12381G1Mayo1Pub
    }
...
impl<'a> ConvView for View<'a> {
    fn to_public_key(&self) -> Result<Multikey, Error> {
        ...
        let kp = KeyPair::<Mayo1>::from_seed(&secret_bytes[bls::BLS_G1_SECRET_LEN..])
            .map_err(|e| ConversionsError::SecretKeyFailure(format!("MAYO-1 seed error: {}", e)))?;
        ...
        Builder::new(Codec::Bls12381G1Mayo1Pub)
    ...
    fn to_ssh_public_key(&self) -> Result<ssh_key::PublicKey, Error> {
        Err(ConversionsError::UnsupportedAlgorithm(
            "BLS12-381-G1-MAYO-1 not supported in SSH key format".into(),
        )
        .into())
    }
...
impl<'a> SignView for View<'a> {
    fn sign(&self, msg: &[u8], combined: bool, _scheme: Option<u8>) -> Result<Multisig, Error> {
        ...
        let mut ms = ms::Builder::new(Codec::Bls12381G1Mayo1Msig).with_signature_bytes(&sig_bytes);
        if combined {
...
impl<'a> VerifyView for View<'a> {
    fn verify(&self, multisig: &Multisig, msg: Option<&[u8]>) -> Result<(), Error> {
        ...
        // Verify MAYO-1: verify(m || s1, s2)
        let vk = VerifyingKey::<Mayo1>::try_from(mayo_pub_bytes)
            .map_err(|_| ConversionsError::PublicKeyFailure("invalid MAYO-1 public key".into()))?;
        let signature = Signature::<Mayo1>::try_from(s2_bytes)
            .map_err(|_| VerifyError::BadSignature("invalid MAYO-1 signature".into()))?;
        ...
        vk.verify(&m2, &signature)
            .map_err(|e| VerifyError::BadSignature(format!("MAYO-1 verify failed: {}", e)))?;
...
mod tests {
    ...
    fn test_sign_verify_roundtrip() {
        ...
        let sk = Builder::new_from_random_bytes(Codec::Bls12381G1Mayo1Priv, &mut rng)
            .unwrap()
            .try_build()
            .unwrap();
        let pk = sk.conv_view().unwrap().to_public_key().unwrap();

        let msg = b"hello BLS12-381-G1-MAYO-1 hybrid (Birds of Prey 1)!";
